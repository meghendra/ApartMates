...
public class FileManager implements Serializable {
    ...
    public FacesMessage storePhotoFile(UploadedFile file) {
        try {
            ...
            Integer roommateID = (int) FacesContext.getCurrentInstance()
                    .getExternalContext().getSessionMap().get("roommateID");

            // Obtain the object reference of the logged-in roommate object
            Roommate r = roommateFacade.findById(roommateID);
            ...
            Photo newPhoto = new Photo(fileExtension, r);

            // Create a record for the new Photo object in the database
            photoFacade.create(newPhoto);

            Photo photo = photoFacade.findPhotosByRoommateID(r.getRoommateID()).get(0);
